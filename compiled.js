(function() {
  ...
  Controller.prototype.setCanvasCursor = function(cssCursor) {
    this._canvas.style.cursor = cssCursor;
  };
  ...
  DragMouseBehavior.prototype.down = function(x, y) {
    ...
    this._controller.setCanvasCursor('move');
  };
  ...
  DragMouseBehavior.prototype.up = function(x, y) {
    ...
    this._onDownGridSpace = null;
    this._onDownViewport = null;
    this._controller.setCanvasCursor('default');
  };

  DragMouseBehavior.prototype.scroll = function(x, y, deltaX, deltaY) {
  };
  ...
  ZoomMouseBehavior.prototype.scroll = function(x, y, deltaX, deltaY) {
    this._controller.viewport = Zoom.zoomViewport(this._controller.viewport, deltaY, new Vector(x, y), this._controller.canvasSize());
  };
  ...
  PanTouchHandler.prototype.touchStart = function(e) {
    ...
    this._panning = in_List.count(in_HTMLTouchEvent.touches(e)) == 1;
  ...
  PanTouchHandler.prototype.touchMove = function(e) {
    ...
    this._panning = this._panning && in_List.count(in_HTMLTouchEvent.touches(e)) == 1;
  ...
  ZoomTouchHandler.prototype._onZoom = function(e) {
    ...
    this._controller.viewport = Zoom.zoomViewport(this._onStartViewport, this._onStartDistance.value - this._distanceBetweenTouches(e), this._touchCenter(e), this._controller.canvasSize());
  };
  ...
  Zoom.zoomViewport = function(viewport, delta, canvasSpaceCenter, canvasSize) {
    ...
    if ((newSize.x < Zoom.minSize || newSize.y < Zoom.minSize) && delta < 0) {
      newSize = new Vector(Zoom.minSize, Zoom.minSize * viewportAspectRatio);
    }

    else if ((newSize.x > Zoom.maxSize || newSize.y > Zoom.maxSize) && delta > 0) {
      newSize = new Vector(Zoom.maxSize, Zoom.maxSize * viewportAspectRatio);
    }
